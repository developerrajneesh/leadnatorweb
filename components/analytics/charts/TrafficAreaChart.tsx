...
const DEFAULT_SIZE = {
  w: 640,
  h: 248,
  pad: { top: 22, right: 16, bottom: 48, left: 52 },
};
const COMPACT_SIZE = {
  w: 420,
  h: 172,
  pad: { top: 14, right: 10, bottom: 34, left: 38 },
};
...
export default function TrafficAreaChart({
  ...
  const chart = useMemo(() => {
    ...
    return { points, line, area, yTicks, innerH, maxY, hitW, xTickIndices, compactLabels, w, h, pad };
  }, [series, size]);
  ...
  const clipId = `sa-area-clip-${uid}`;
  const fillId = `saAreaFill-${uid}`;
  const strokeId = `saLineStroke-${uid}`;
  const { w, h, pad } = chart;

  return (
    <div className={`sa-viz sa-viz-area${compact ? " sa-viz-area-compact" : ""}`}>
      {!hideSummary && (
          ...
          {chart.points.map((p, i) =>
            p.count > 0 && hover === i ? (
              <g key={`dot-${p.date}`}>
                ...
                />
                <circle cx={p.x} cy={p.y} r={compact ? 4 : 5} className="sa-viz-dot" />
                <circle cx={p.x} cy={p.y} r={compact ? 7 : 9} className="sa-viz-dot-ring" />
              </g>
            ) : null,
          )}
        </g>
      ...
      {hideSummary && active && (
        <div className="sa-viz-hover-chip">
          <strong>{active.count.toLocaleString()}</strong>
          <span>{formatLabel(active.date)}</span>
        </div>
      )}
